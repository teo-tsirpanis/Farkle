...
* The precompiler API has substantially changed, to be more explicit and friendly with trimming and AOT compilation. The main differences are:
  * Instead of declaring a static readonly field of type `PrecompilableDesigntimeFarkle<T>`, marking a grammar for precompilation is now done by declaring a static method that returns the grammar builder, and applying @"Farkle.Builder.PrecompilerInputAttribute" to it.
  * Instead of directly building a `PrecompilableDesigntimeFarkle<T>`, obtaining a precompiled grammar is now done by declaring a static method that returns a `CharParser<T>`, applying @"Farkle.Builder.PrecompilerOutputAttribute" to it, and calling it. The precompiler will replace its body with code that loads the precompiled grammar.
* The precompiler is no longer supported in Visual Studio 2022 and earlier.

Here's a comparison of using the precompiler in Farkle 6.x versus Farkle 7.0:
...
```csharp
var parser = MyGrammars.MyParser;
Console.WriteLine(parser.Parse("12345"));

public static class MyGrammars
{
    public static readonly PrecompilableDesigntimeFarkle<int> MyGrammar = BuildMyGrammar(…)
        .MarkForPrecompile();

    public static readonly RuntimeFarkle<int> MyParser = MyGrammar.Build();
}
```
...
{
    [PrecompilerInput]
    public static IGrammarBuilder<int> GetMyGrammar() => BuildMyGrammar(…);

    [PrecompilerOutput]
    public static CharParser<int> MyParser = CharParser.MustPrecompile<int>();
}
```
...
[<PrecompilerOutput>]
let getMyParser() = CharParser.mustPrecompile<int>

[<EntryPoint>]
let main _ =
    let parser = getMyParser()
    printfn "%A" <| parser.Parse "12345"
    0
```
...
* The representation of token symbols produced by a DFA has been streamlined. Instead of a strongly-typed choice between different symbol types, all kinds of symbols (terminals, comments, group boundaries, etc.) are now represented by the @"Farkle.Grammars.TokenSymbolDefinition" type.
