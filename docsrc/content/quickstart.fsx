...
This grammar will have seven terminals: the number, and each of the math symbols, and the parentheses. In Farkle, we create terminals from hard-coded regular expressions (regexes). Let's see how a regular expression that recognizes a decimal number looks like:
*)
...
The terminal we created is of type `DesigntimeFarkle<float>`. This means that we can theoretically use it to parse floating-point numbers from text. But we want to create something bugger than that. As we are going to see, we can compose designtime Farkles into bigger ones, using nonterminals.
