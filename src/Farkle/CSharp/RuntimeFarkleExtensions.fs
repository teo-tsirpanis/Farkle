...
type RuntimeFarkleExtensions =
    ...
    static member Parse(rf, cs) =
        RF.parseChars rf cs

    [<Extension>]
    /// <summary>Parses and post-processes a string.</summary>
    /// <param name="str">The string to parse.</param>
    static member Parse(rf, str) =
        RF.parseString rf str
    ...
    static member Parse(rf, mem) =
        RF.parseMemory rf mem
    ...
    static member Parse(rf, stream, [<Optional>] encoding, [<Optional; DefaultParameterValue(true)>] doLazyLoad) =
        let encoding = if isNull encoding then Encoding.UTF8 else encoding
        RF.parseStream rf doLazyLoad encoding stream
    ...
    static member Parse(rf, textReader) =
        RF.parseTextReader rf textReader
    ...
    static member ParseFile(rf, fileName) =
        RF.parseFile rf fileName

    [<Extension>]
    /// <summary>Returns a new <see cref="RuntimeFarkle{TResult}"/>
    /// with a changed <see cref="PostProcessor"/>.</summary>
    static member ChangePostProcessor(rf: RuntimeFarkle<'TResult>, pp: PostProcessor<'TNewResult>) =
        RF.changePostProcessor pp rf

    [<Extension>]
    /// <summary>Returns a <see cref="RuntimeFarkle{Object}"/>
    /// that just checks if its given input is valid.</summary>
    /// <remarks>If syntax-checking succeeds, the value of
    /// the result will be always <c>null</c></remarks>
