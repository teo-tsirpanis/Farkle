...
module PostProcessor =
    ...
    let ast =
        {new PostProcessor<AST> with
            ...
            member __.Fuse (prod, items) =
                items |> Seq.cast |> List.ofSeq |> curry AST.Nonterminal prod |> box}
    ...
    let ofSeq<'result> transformers fusers =
        ...
        {new PostProcessor<'result> with
            member __.Transform (sym, pos, data) =
                ...
                match transformers.TryGetValue(sym.Index, &f) with
                | true -> f.Invoke(pos, data)
                | false -> null
